...
import { ContaPagarFormDialog } from "./conta-pagar-form-dialog";
import { MarcarPagaDialog } from "./marcar-paga-dialog";
import { STATUS_CONTA_LABELS, StatusContaBadge, effectiveStatus } from "./financeiro-labels";
...
export function ContasPagarTable({
  ...
  const [editTarget, setEditTarget] = useState<ContaPagar | null>(null);
  const [pagaTarget, setPagaTarget] = useState<ContaPagar | null>(null);
  ...
  function handleSaved(conta: ContaPagar) {
    setContas((prev) => {
  ...
  async function handleDelete() {
    ...
    const { error } = await supabase.from("contas_pagar").delete().eq("id", deleteTarget.id);
    if (error) {
  ...
  async function refreshPaga() {
    ...
    const supabase = createClient();
    const { data } = await supabase.from("contas_pagar").select("*").eq("id", pagaTarget.id).single();
    if (data) handleSaved(data as ContaPagar);
  }
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          title={contas.length === 0 ? "Nenhuma conta a pagar" : "Nenhum resultado"}
          description={
            contas.length === 0
            ...
            <tbody>
              {filtered.map((conta) => {
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3.5 font-medium text-white">{conta.fornecedor}</td>
                    <td className="px-5 py-3.5 text-slate">{categoriaNome(conta.categoria_id)}</td>
